	...
	for(var/job_map in SSjobs.job_lists_by_map_name)
		...
		if(!pref.hiding_maps[job_map])
			...
			for(var/datum/job/job in map_job_list)
				...
				else if(LAZYACCESS(job.minimum_character_age, bodytype) && user.client && (user.client.prefs.age < job.minimum_character_age[bodytype]) && config.use_char_age_restriction_for_jobs)
