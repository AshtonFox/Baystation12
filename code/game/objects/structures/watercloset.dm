...
/obj/structure/hygiene/shower/proc/update_sound(var/playing)
	if(playing && !sound_token)
		sound_token = GLOB.sound_player.PlayLoopingSound(src, "\ref[src]", 'frontier/sound/machines/shower_mid.ogg', 50, 3)
...
/obj/structure/hygiene/shower/attack_hand(var/mob/M)
	...
	update_icon()
	if(on)
		playsound(src, 'frontier/sound/machines/shower_start.ogg', 50, 3)
		update_sound(1)
		if (M.loc == loc)
			wash(M)
			process_heat(M)
		...
		update_sound(0)
		playsound(src, 'frontier/sound/machines/shower_end.ogg', 50, 3)
